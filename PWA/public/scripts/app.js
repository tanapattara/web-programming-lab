...
function renderForecast(card, data) {
  ...
  card.querySelector('.description').textContent = data.name;
  ...
  card.querySelector('.current .icon')
    .className = `icon ${data.weather[0].main}`;
...
function loadLocationList() {
  ...
  if (!locations || Object.keys(locations).length === 0) {
    const key = '17.8057372,102.7470427';
    locations = {};
    locations[key] = {
      label: 'Khon Kaen University, Nong Khai Campus',
      geo: '17.8057372,102.7470427',
      lat: '17.8057372',
      lon: '102.7470427'
    };
  }
